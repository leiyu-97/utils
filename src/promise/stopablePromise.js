...
class StopablePromise extends Promise {
   ...
   * @return {StopablePromise} Promise
   */
  then(resCb, rejCb) {
    let promise;
    ...
    const resolve = resCb ? (...param) => {
      ...
      if (result instanceof Promise) {
        result = new StopablePromise(result);
        promise._res = result;
      }
      return result;
    } : undefined;
  ...
  static wrap(promise) {
    return new StopablePromise(promise.then.bind(promise));
  }
}

module.exports = StopablePromise;
