...
function formatTime(date, format) {
  ...
  const result = format.replace(/[a-zA-Z]/gm, (char) => {
    switch (char) {
      case 'd':
        return date
          .getDate()
      ...
      case 'S':
        ...
        return ['th', 'st', 'nd', 'rd', 'th', 'th', 'th', 'th', 'th', 'th'][num[num.length - 1]];
      case 'w':
        return date.getDay();
      case 'z':
        return Math.floor((date - new Date(`${date.getFullYear()}-01-01`)) / DAY) + 1;
      case 'W':
        year = new Date(`${date.getFullYear()}-01-01`);
        num = year.getDay();
        num = num === 0 ? 6 : num - 1;
        return Math.floor((date - year + num * DAY) / WEEK) + 1;
      case 'F':
        return [
          'January',
          'February',
      ...
      case 't':
        ...
        return ['31', Math.ceil(num) - num ? '28' : '29', '31', '30', '31', '30', '31', '31', '30', '31', '30', '31'][
          date.getMonth()
        ];
      case 'L':
        num = date.getFullYear() / 4;
